...
# 2012年から2028年までの国債残高÷名目GDPの割合の予測値を計算
years_future = np.array(range(2012, 2029))
...
# グラフの表示
plt.bar(years_future, predicted_balance_gdp_ratio_future)
plt.xlabel("Year")
plt.ylabel("Balance/GDP Ratio")
plt.title("Predicted Balance/GDP Ratio from 2012 to 2028")
plt.show()

# 予測値の出力
for year, ratio in zip(years_future, predicted_balance_gdp_ratio_future):
    print(f"{year}年の国債残高÷名目GDPの予測値: {ratio:.4f}")
