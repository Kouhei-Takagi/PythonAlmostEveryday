...
# 銅価格を3乗に正規化
copper_closePow3 = copper_close ** 3

# 終値データの相関係数を計算
correlation = copper_closePow3.corr(dollar_nikkei_close)

# データをプロット
plt.figure(figsize=(14,8))
plt.plot(copper_closePow3.index, copper_closePow3, label='Copper Close Pow 3')
plt.plot(dollar_nikkei_close.index, dollar_nikkei_close, label='Nikkei Close (USD)')

plt.title('Copper Pow 3 vs Nikkei (in USD)')
plt.xlabel('Date')
plt.ylabel('Price')
plt.legend()
plt.show()

plt.savefig("./Nikkei225vsCopper3/copperPow3vsNikkei225USdollar-1.png")

# グラフ化
plt.scatter(copper_closePow3, dollar_nikkei_close)
plt.xlabel("Copper Close Pow 3 Price")
...
plt.savefig("./Nikkei225vsCopper3/copperPow3vsNikkei225USdollar-2.png")
