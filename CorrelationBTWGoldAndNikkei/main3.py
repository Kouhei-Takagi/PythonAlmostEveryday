...
end_date = "2023-05-26"
silverData = yf.download(symbol, start=start_date, end=end_date)

# 日経平均株価のデータの取得
symbol = "^N225"
start_date = silverData.index[0].strftime("%Y-%m-%d")  # silverデータの最初の日付を取得
...
# 終値の列のみを抽出
silver_close = silverData["Close"]
nikkei_close = nikkeiData["Close"]
exchange_rate = exchangeData["Close"]

# データの長さ確認
print("silver Close:", len(silver_close))
print("Nikkei Close:", len(nikkei_close))
print("Exchange Rate:", len(exchange_rate))

# データの結合と欠損値の除去
merged_data = pd.concat([silver_close, nikkei_close, exchange_rate], axis=1, join="inner")
merged_data.columns = ["silver Close", "Nikkei Close", "Exchange Rate"]

# 終値データを再取得
silver_close = merged_data["silver Close"]
...
# 終値データの相関係数を計算
correlation = silver_close.corr(dollar_nikkei_close)

# グラフ化
plt.scatter(silver_close, dollar_nikkei_close)
plt.xlabel("silver Close Price")
